...
export default function Themes(props) {
  return (
    <>
    <label className={`form-check-label text-${props.mode === 'dark'?'light':'dark'}`} htmlFor="btn-check-outlined">{props.modeName === 'Dark'?'':'Light'} &nbsp;</label>
    <div className="form-check form-switch form-check-inline">
        <input className="form-check-input" onClick={props.toggleMode} type="checkbox" role="switch" name='flexDefault' id="btn-check-outlined" value="option1" />
        <label className={`form-check-label text-${props.mode === 'dark'?'light':'dark'}`} htmlFor="btn-check-outlined">{props.modeName === 'Light'?'':'Dark'}</label>
    </div>
